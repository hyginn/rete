#SEL3D.R
#
#' \code{getPdbIds} BLASTs the input cDNA sequence against PDB and returns an MT object
...
#' @param convert bool to convert DNA sequences to amino acids, True by default, False for recursion
...
getPdbIds <- function(targetSeq, startPos = 1 , endPos = 1, convert = TRUE, silent = TRUE){
  ...
  if(silent) {
    suppressWarnings(capture.output(blastOutput <- bio3d::blast.pdb(AAseq)))
  } else {
