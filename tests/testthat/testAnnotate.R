...
# normal gH hashtable
names = c("ENSP00000000233", "ENSP00000005340", "ENSP00000003100") # Unknown: "ENSP00000005257", "ENSP00000003084"
heats = c(0.3, 0.5, 0.2)
...
test_that("annotateGraph rejects inappropriate objects", {
    # test null gH object
    null_gH <- NULL
    expect_error(annotateGraph(gH = null_gH, gG = test_gG, writeLog = FALSE), "Object \"null_gH\" is NULL.")

    # test null gG object
    null_gG <- NULL
    expect_error(annotateGraph(gH = test_gH, gG = null_gG, writeLog = FALSE), "Object \"null_gG\" is NULL.")
    ...
    expect_error(annotateGraph(gH = test_gH, gG = test_gG, writeLog = "notlogical"), "Error: writeLog must be of mode type and class logical")

    # test silent is logical
    expect_error(annotateGraph(gH = test_gH, gG = test_gG, silent = "notlogical"), "Error: silent must be of mode type and class logical")
})
...
test_that("annotateGraph returns a graph which is identical to the original gG, except for the heat attribute", {
    test_AGG <- annotateGraph(gH = test_gH, gG = test_gG, writeLog = FALSE)
    # test number of vertices
    expect_equal(igraph::vcount(test_gG), igraph::vcount(test_AGG))

    # test order of vertices
    expect_equal(igraph::vertex_attr(test_gG, "name"), igraph::vertex_attr(test_AGG, "name"))

    # test number of edges
    expect_equal(igraph::ecount(test_gG), igraph::ecount(test_AGG))

    # test edge weights
    expect_equal(igraph::edge_attr(test_gG, "weight"), igraph::edge_attr(test_AGG, "weight"))
...
#   # left out for now - needs a helper function since this will be tested often for other functions
# })
...
test_that("annotateGraph does not produce a broken AGG if it receives a broken input", {
    ...
    test_null_heat_AGG <- annotateGraph(gH = testgHNullHeat, gG = test_gG, writeLog = FALSE)

    # that all heats are the same except for third heat value
    expect_equal(igraph::vertex_attr(test_null_heat_AGG)$heat[1:2], igraph::vertex_attr(normal_AGG)$heat[1:2])
    expect_equal(igraph::vertex_attr(test_null_heat_AGG)$heat[4:5], igraph::vertex_attr(normal_AGG)$heat[4:5])
})
